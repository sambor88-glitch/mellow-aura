...
/**
 * Vouchers added from their product page. The name and dedication are optional, and their limits
 * come from the panel.
 */
class VoucherLines extends ProductLines
{
    public function fromRequest(Request $request): CartLine
    {
        ...
        return new VoucherLine(
            VoucherLine::keyForVoucher($variant, $recipientName, $dedication),
            (int) ($data['quantity'] ?? 1),
            $variant,
            $recipientName,
            $dedication,
    ...
    {
        return new VoucherLine($key, $quantity, $variant, $row['recipient_name'] ?? null, $row['dedication'] ?? null);
    }
}
