...
@if ($vouchers->isNotEmpty())
    ...
    @foreach ($vouchers as $voucher)
        <p style="margin: 0; color: #2F2620;">
            <strong style="font-weight: 600; letter-spacing: 1px;">{{ $voucher->code }}</strong>{{ $voucher->recipient_name ? ' · dla: '.$voucher->recipient_name : '' }} · ważny do {{ $voucher->valid_until->translatedFormat('j F Y') }}
        </p>
    @endforeach
