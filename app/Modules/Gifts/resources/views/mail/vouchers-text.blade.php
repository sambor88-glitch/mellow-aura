...
- {!! $voucher->code !!}: {!! collect([$voucher->orderItem?->product_name, $voucher->orderItem?->variant_label, $voucher->recipient_name ? 'dla: '.$voucher->recipient_name : null, 'ważny do '.$voucher->valid_until->translatedFormat('j F Y')])->filter()->join(', ') !!}
@endforeach

Numer vouchera wystarczy, żeby go wykorzystać.@if ($contactEmail) Pytanie? Odpisz na tego maila — trafi prosto do mnie.@endif
