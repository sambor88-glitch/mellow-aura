...
@endphp
{{-- Inside the product's add-to-cart form: the names and dedication go with the voucher, and the preview shows the printed card. --}}
<input type="hidden" name="type" value="voucher">
<div x-data="{ recipient: '', sender: '', dedication: '' }" class="mb-[26px] rounded-[4px] border border-line bg-cream px-5 py-[22px]">
    ...
    <label for="voucher-from" class="mb-1.5 block text-[13.5px] text-graphite">Od kogo</label>
    <input id="voucher-from" name="sender_name" x-model="sender" maxlength="{{ $nameMax }}" placeholder="np. Kasia i Tomek" autocomplete="off"
           aria-describedby="voucher-from-hint"
           class="w-full min-w-0 rounded-[4px] border border-line bg-white px-[15px] py-[13px] text-[16px] text-ink placeholder:text-hint focus:border-ink">
    <div id="voucher-from-hint" class="mt-2 mb-4 flex justify-between gap-3 text-[12.5px] text-hint">
        <span>Puste pole zostawi na voucherze linię do podpisu</span>
        <span class="flex-none whitespace-nowrap" x-text="sender.length + ' / {{ $nameMax }}'">0 / {{ $nameMax }}</span>
    </div>
    ...
    <div class="rounded-[4px] border border-divider bg-sand px-[22px] py-7 text-center">
        ...
        <div x-show="sender.trim()" x-cloak class="mb-3 text-[14px] text-graphite [overflow-wrap:anywhere]">od: <span x-text="sender.trim()"></span></div>
