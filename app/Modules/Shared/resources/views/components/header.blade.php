...
@endphp
<div class="sticky top-0 z-60 border-b border-divider bg-[rgba(243,237,228,0.94)] backdrop-blur-[14px]">
    <div class="mx-auto flex max-w-[1280px] flex-wrap items-center gap-5 px-7 py-4">
        ...
        <nav aria-label="Główne menu" class="flex min-w-0 flex-[1_1_200px] flex-wrap gap-5 text-[13.5px] tracking-[0.02em]">
            @foreach ($links as [$label, $route, $pattern])
                @if (Route::has($route))
                    <a href="{{ route($route) }}" @if (request()->routeIs($pattern)) aria-current="page" @endif class="text-[#4A3E33] transition-colors duration-250 hover:text-navy">{{ $label }}</a>
                @endif
            @endforeach
        </nav>
    </div>
