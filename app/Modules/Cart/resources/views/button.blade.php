...
<button type="button" x-data x-on:click="$store.cart.open($el)" aria-haspopup="dialog" aria-controls="cart"
        class="fill-btn ml-auto flex min-h-11 items-center gap-2.5 rounded-full bg-ink px-5 py-[11px] text-[13px] tracking-[0.04em] text-linen transition duration-300 hover:bg-rose hover:text-ink active:scale-[.97]">
    <span>{{ __('cart::drawer.button') }}</span>
